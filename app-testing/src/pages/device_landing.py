...
class DeviceLanding:
    ...
    def calibrate_now(self, pipette: str) -> Element:
        """Click calibrate now button by pipette model."""
        # will not handle 2 pipettes of the same model
        return Element(
            (
                By.XPATH,
    ...
    def click_calibrate_now(self, pipette: str) -> None:
        """Return_tip_exit button."""
        self.base.click(self.calibrate_now(pipette=pipette))

    continue_with_calibration_block: Element = Element(
        (By.XPATH, '//button[text()="Continue with calibration block"]'),
        "Continue with calibration block button.",
    )

    def click_continue_with_calibration_block(self) -> None:
        """Return_tip_exit button."""
        self.base.click(self.continue_with_calibration_block)
